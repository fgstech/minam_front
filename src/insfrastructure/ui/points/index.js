...
import "./style.css";
import XpPointsAPI from '../../../api/xp';

const Points = ({ type = "nam" }) => {
    const [points, setPoints] = useState("0");
    const format = (numero) => numero.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');

    const getNamPoinst = async (userId) => {
        const res = await PointsAPI.getWallet(userId);
        const { balance } = res.data.data;
        return balance;
    }

    const getXpPoints = async (userId) => {
        const { wallet } = await XpPointsAPI.getWallet(userId);
    ...
    useEffect(async () => {
        try {
            ...
            let balance = 0;
            if (type === "nam") {
                balance = await getNamPoinst(userId);
                setPoints(format(balance));
            } else if (type === "xp") {
                balance = await getXpPoints(userId);
                setPoints(format(balance));
            }
        } catch (err) {
            setPoints(format(0));
    ...
    return (
        <div className={`points-container ${type}`}>
            {type === "nam" ? <>
            ...
            <span>{format(points)} {type === "xp" ? "XP" : null}</span>
        </div>
    );
};
